...
(defmulti string-length-fn
  "Return a HoneySQL form appropriate for getting the length of a `Field` identified by fully-qualified `field-key`. An
  implementation should return something like:

      (hsql/call :length (hx/cast :VARCHAR field-key))"
  {:arglists '([this field-key])}
  driver/dispatch-on-driver
  :hierarchy #'driver/hierarchy)
